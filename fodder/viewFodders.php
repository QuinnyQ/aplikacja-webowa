...
FROM pasze p
JOIN zwierzęta z ON p.id_zwierze = z.id";
...
$result = $conn->query($sql);
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo        'Nazwa paszy: ' . $row["nazwa_paszy"];
            echo      '</h5>';
            echo      '<p class="card-text">';
            echo        'Oznaczenie: ' . $row["oznaczenie"];
            echo      '</p>';
            echo      '<p class="card-text">';
            echo        'Ilość: ' . $row["ilosc"] . " kg";
            echo      '</p>';
            echo      '<p class="card-text">';
            echo        'Nazwa zwierzęcia: ' . $row["nazwa"];
            echo      '</p>';
            echo      '<p class="card-text">';
            ...
            echo      '</p>';
            echo      '<a href="http://www.kingapabinczyk.cba.pl/fodder/viewFodder.php?id=' . $row["id"] . '" class="btn btn-primary">Zobacz więcej</a>';
            echo    '</div>';
            echo  '</div>';
            echo '</div>';
        }
